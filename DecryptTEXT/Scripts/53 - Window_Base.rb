...
class Window_Base < Window
  ...
  def initialize(x, y, width, height)
    ...
    if $game_switches[1317] == true
    self.back_opacity = $game_variables[4160]
    end
    update_padding
    update_tone
    create_contents
  ...
  def process_draw_icon(icon_index, pos)
    #if $game_switches[803] == true && icon_index == 626
    #  icon_index = 555
    #end  
  ...
  # * Draw Class
  #--------------------------------------------------------------------------
  def draw_actor_class(actor, x, y, width = 200)
    change_color(normal_color)
  ...
  # * Draw Nickname
  #--------------------------------------------------------------------------
  def draw_actor_nickname(actor, x, y, width = 280)
    change_color(normal_color)
  ...
  def draw_actor_level(actor, x, y)
    ...
    change_color(normal_color)
    draw_text(x + 32, y, 32, line_height, actor.level, 2)
  end
  #--------------------------------------------------------------------------
  # * Draw State and Buff/Debuff Icons
  #--------------------------------------------------------------------------
  def draw_actor_icons(actor, x, y, width = 240)#96 before
    icons = (actor.state_icons + actor.buff_icons)[0, width / 24]
    icons.each_with_index {|n, i| draw_icon(n, x + 24 * i, y + 24) }
  end
  #--------------------------------------------------------------------------
  ...
  def draw_current_and_max_values(x, y, width, current, max, color1, color2)
    ...
    xr = x + width
    if width < 96
      draw_text(xr - 82, y, 84, line_height, current, 2)
    else
      draw_text(xr - 144, y, 72, line_height, current, 2)
      change_color(color2)
      draw_text(xr - 68, y, 14, line_height, "/", 2)
      draw_text(xr - 72, y, 72, line_height, max, 2)
    end
  end
  #--------------------------------------------------------------------------
  # * Draw HP
  #--------------------------------------------------------------------------
  def draw_actor_hp(actor, x, y, width = 186)
  ...
  # * Draw MP
  #--------------------------------------------------------------------------
  def draw_actor_mp(actor, x, y, width = 186)
  ...
  # * Draw Exp
  #--------------------------------------------------------------------------    
   def draw_actor_exp(actor, x, y, width = 186)
     change_color(system_color)
     draw_text(x, y, 180, line_height, "Next Level")
     s1 = actor.max_level? ? "-----" : actor.next_level_exp - actor.exp
     change_color(normal_color)
     draw_text(x+40, y, 180, line_height, s1, 2)
     #draw_current_and_max_values(x, y, width, actor.exp_for_level(actor.level+1),
     # mp_color(actor), normal_color)
    end  
    
    
   def draw_exp_info00000(x, y)
  ...
  # * Draw TP
  #--------------------------------------------------------------------------
  def draw_actor_tp(actor, x, y, width = 186)
  ...
  #     enabled : Enabled flag. When false, draw semi-transparently.
  #--------------------------------------------------------------------------
  def draw_item_name(item, x, y, enabled = true, width = 272)#172)
    return unless item
