...
module YEA
  module ITEM
    ...
    ITEM_TYPES =[
    # [  :symbol,   "Display"],
    #  [   :field,     "Usable"], # Shows Menu-usable items.
    #  [  :battle,    "Battle"], # Shows Battle-usable items.
      [:category,   "Usable"], # Categorized by <category: string>
      [:category,   "Consumable"], # Categorized by <category: string>
      [:category,   "Food"], # Categorized by <category: string>      
      [:category,   "Diary"], # Categorized by <category: string>      
      [:category,   "Key"], # Categorized by <category: string> 
      [:category,   "Material"], # Categorized by <category: string>
      [:category,   "Bone"], # Categorized by <category: string>       
      [:category,   "Book"], # Categorized by <category: string> 
     # [:category,   "Dowry"], # Categorized by <category: string>     
      [:category,   "Debug"], # Categorized by <category: string>
    ...
    VOCAB_STATUS ={
     ...
     # :tp_recover => "Calories",      # Text used for TP Recovery.
      ...
      when :types
        ...
        when :weapon
          for i in 1...$data_system.weapon_types.size
            ...
            if name != "Empty" && name != "Cursed Axe" && name != "Bow"
...
class Window_ItemStatus < Window_Base
  ...
  def draw_item_effects
    ...
    draw_applies(dx + dw, dy + line_height * 0, dw)
    draw_removes(dx + dw, dy + line_height * 1, dw)
...
class Scene_Item < Scene_ItemBase
  ...
  #--------------------------------------------------------------------------
  def open_types
    return unless @category_window && @types_window
