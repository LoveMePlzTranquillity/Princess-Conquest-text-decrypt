...
class Window_Selectable < Window_Base
  ...
  def process_cursor_move
   ...
   if !WolfPad.plugged_in?
    ...
    cursor_down (Input.trigger?(:DOWN)) if Keys.trigger?(:DOWN) || WolfPad.repeat?(:L_DOWN,0) || WolfPad.repeat?(:DOWN,0) || WolfPad.repeat?(:L_DOWN,1) || WolfPad.repeat?(:DOWN,1)
    cursor_up   (Input.trigger?(:UP))    if Keys.trigger?(:UP) || WolfPad.repeat?(:L_UP,0) || WolfPad.repeat?(:UP,0) || WolfPad.repeat?(:L_UP,1) || WolfPad.repeat?(:UP,1)
  ...
  def process_handling
    ...
    return process_ok       if ok_enabled?  && Input.trigger?(:C) or ok_enabled? && WolfPad.trigger?(:X,0) or ok_enabled? && WolfPad.trigger?(:X,1)
    return process_cancel   if cancel_enabled?    && Input.trigger?(:B) or cancel_enabled? && WolfPad.trigger?(:B,0) or ok_enabled? && WolfPad.trigger?(:B,1) or ok_enabled? && WolfPad.trigger?(:START,0) or ok_enabled? && WolfPad.trigger?(:START,1)
