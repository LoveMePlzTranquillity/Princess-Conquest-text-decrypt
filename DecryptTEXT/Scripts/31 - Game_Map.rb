...
class Game_Map
  ...
  def setup_parallax
    ...
    if @map.parallax_name == "Tavern"
      if RTime::month == 12 && RTime::day >= 19
      @parallax_name = "TavernXMAS"
      elsif RTime::month == 1 && RTime::day <= 6 
      @parallax_name = "TavernXMAS"
      elsif $game_switches[4333] == true
      @parallax_name = "TavernXMAS"
      end
    end  
  ...
  def setup_autorun_common_event
    ...
    event = $data_common_events.find do |event|
      ...
      ###
      #if event && event.autorun? && $game_switches[event.switch_id]
