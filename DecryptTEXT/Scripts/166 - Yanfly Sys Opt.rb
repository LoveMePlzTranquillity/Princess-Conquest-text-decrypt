...
module YEA
  module SYSTEM
    ...
    DEFAULT_ANIMATIONS = false    # Enable battle animations by default?
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # - Command Settings -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    COMMANDS =[
      ...
      :window_grn,   # Changes the green tone for all windows.
      :window_blu,   # Changes the blue tone for all windows.
      :switch_4,     # Custom Switch 4. Adjust settings below.     
      :variable_3,   # Custom Variable 3. Adjust settings below.  
      :variable_7,   # Custom Variable 6. Adjust settings below.  
      :variable_5,   # Custom Variable 3. Adjust settings below.  
      :blank,
      :volume_bgm,   # Changes the BGM volume used.
      :volume_bgs,   # Changes the BGS volume used.
      :volume_sfx,   # Changes the SFX volume used.
      :variable_6,   # Custom Variable 6. Adjust settings below.      
      :blank,
      :autodash,     # Sets the player to automatically dash.
    # :instantmsg,   # Sets message text to appear instantly.
    # :animations,   # Enables battle animations or disables them.
     :switch_1,     # Custom Switch 1. Adjust settings below.
     :switch_8,     # Custom Switch 8. Adjust settings below.
     :switch_7,     # Custom Switch 7. Adjust settings below.  
     :switch_2,     # Custom Switch 2. Adjust settings below.
     :switch_6,     # Custom Switch 6. Adjust settings below.     
     :switch_3,     # Custom Switch 3. Adjust settings below.  
     :switch_9,     # Custom Switch 9. Adjust settings below.  
     :switch_10,     # Custom Switch 10. Adjust settings below.  
    # :switch_5,     # Custom Switch 5(controls). Adjust settings below.       
     :variable_1,   # Custom Variable 1. Adjust settings below.
     :variable_2,   # Custom Variable 2. Adjust settings below.
     :variable_4,   # Custom Variable 4. Adjust settings below.
     :blank,
      :to_title,     # Returns to the title screen.
      :shutdown,     # Shuts down the game.
    ] # Do not remove this.
    ...
    #               ], # Do not remove this.
    # -------------------------------------------------------------------------
      :switch_1  => [ 2584, "Battle-Fuck", "YES", "NO",
                     "Choose if you want Battle-Fuck mechanics."
                    ],
    # -------------------------------------------------------------------------
      :switch_2  => [ 1788, "Conquest Ends", "YES", "NO",
                     "Choose if you want Conquest Endings to happen when a Reign gets too powerful."
                    ],
    # -------------------------------------------------------------------------
      :switch_3  => [ 3841, "Autosaves", "YES", "NO",
                     "Choose if you want the game to be automatically saved in Slot 16 every new day."
                    ],       
 # -------------------------------------------------------------------------
      :switch_4  => [ 1317, "Window Opacity", "Default", "Custom",
                     "Select if you want to use Default or Custom opacity for the game's windows."
                    ],             
 # -------------------------------------------------------------------------
      :switch_5  => [ 1320, "Controls Layout", "Keyboard", "Gamepad",
                     "Select if you want Keyboard or Gamepad controls to be shown."
                    ],                         
    # -------------------------------------------------------------------------
      :switch_6  => [ 6306, "Lewdness Ends", "YES", "NO",
                     "Choose if you want Kingdom Lewdness Ending to happen when the Kingdom gets too corrupted."
                    ],                
    # -------------------------------------------------------------------------
      :switch_7  => [ 4031, "Area CGs", "YES", "NO",
                     "Choose if you want to be shown location artworks when you enter one area."
                    ],                         
    # -------------------------------------------------------------------------
      :switch_8  => [ 4660, "Control Guide", "Show", "Hide",
                     "Choose if you want to show or hide the control guide in the Battle System HUD."
                    ],   
    # -------------------------------------------------------------------------
      :switch_9  => [ 4662, "Male NPCs Animations", "Show", "Skip",
                     "Choose if you to skip male NPCs animations."
                    ],         
    # -------------------------------------------------------------------------
      :switch_10  => [ 4742, "Futa NPCs Animations", "Show", "Skip",
                     "Choose if you to skip futa NPCs animations."
                    ],                        
    # -------------------------------------------------------------------------
    } # Do not remove this.
    ...
    #                ], # Do not remove this.
    # -------------------------------------------------------------------------
      :variable_1 => [ 2766, "Day Length", 9, 1, 1, 16,
                      "Select how many minutes you want an ingame day last for."
                     ],
    # -------------------------------------------------------------------------
      :variable_2 => [ 1913, "Pregnancies Speed", 10, 2, 0, 3,
                      "Select the pregnancies speed multiplier. Selecting 0 stops pregnancies from progressing naturally."
                     ],
 # -------------------------------------------------------------------------
      :variable_3 => [ 4160, "Opacity Level", 11, 2, 0, 255,
                      "Change the opacity for windows.\n" +
                      "Hold \\i[1242]/DASH to change increment by 10."
                     ],                     
    # -------------------------------------------------------------------------
      :variable_4 => [ 7327, "Metabolic Rate", 10, 2, 0, 5,
                      "Select the metabolism speed multiplier. Selecting 0 stops weight gains and losses."
                     ],          
    # -------------------------------------------------------------------------
      :variable_5 => [ 7329, "Screen Filter", 1, 9, 0, 3,
                      "Select a screen filter. 0 = None; 1 = CRT; 2 = VHS-1; 3 = VHS-2;"
                     ],                 
    # -------------------------------------------------------------------------
      :variable_6 => [ 7330, "Animalese Dub", 14, 6, 0, 2,
                      "Select the characters you want to (cutely) speak. 0 = Princesses; 1 = All; 2 = None;"
                     ],     
    # -------------------------------------------------------------------------
      :variable_7 => [ 7334, "Dialogue Box", 0, 7, 0, 11,
                      "0=Default; 1=Dragon; 2=Wooden; 3=Porgy; 4=Dusk; 5=Knight; 6=Bird; 7=Slime; 8=Insect; 9=Skeleton; 10=Mermaid; 11=Candy;"
                     ],                                       
    # -------------------------------------------------------------------------
    } # Do not remove this.
...
class RPG::BGM < RPG::AudioFile
  ...
  #--------------------------------------------------------------------------
  def play(pos = 0)
   #   if $game_switches[7327] == false
   #   $game_switches[7327] = true
   #   $game_variables[7327] = 1
   # end  
    if @name.empty?
      Audio.bgm_stop
      @@last = RPG::BGM.new
...
class RPG::SE < RPG::AudioFile
  ...
  def play
    unless @name.empty?
      ...
      # VOICE SCRIPT #
      @subS = ["Cat1","CatT2","Dog1","Gob1","GolS1","HHS1","HumS1","Ins1","KobS1","Mon1","Mou1","Oni1","Oni2","SliS1","Wyv1"]
      #if $game_variables[7330] != 1 #no Dub 
      #  @subS = [""]
      #end     
      if @subS.include?@name
        @name = "Moan"
        $game_variables[7331] = Array.new(20, 0)
        $game_variables[7331][0] = "RND"    
      end             
      if @name == "Moan"
        pitch += 20
        @name = "Voice" + $game_variables[7331][0]
        @name += (rand(4)+1).to_s     
      end    
      if @name.include?"Hero"
         traitK = $game_variables[628]
         if traitK == 33 || traitK == 133 || traitK == 1033 #Clockwork
          @name = ""
          return
         end
       end      
      if @name == "ROCK"
          @name = ["ROCK1","ROCK2","ROCK3"].sample
        end         
      if @name == "PUNK"
          @name = ["PUNK1","PUNK2","PUNK3"].sample
        end       
      if @name == "BEAT"
          @name = ["Beat1","Beat2","Beat3"].sample
      end         
      # VOICE SCRIPT #     
...
class Game_Player < Game_Character
  ...
  def dash?
    if $game_system.autodash?
      ...
      return !Input.press?(:SHIFT) && !WolfPad.press?(:A,0) && !WolfPad.press?(:A,1)
    else
