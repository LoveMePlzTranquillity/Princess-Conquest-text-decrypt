...
module YSA
  module OVERLAY
    ...
    # Default: ON
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    LIGHT_SWITCH = 1401        # Turn on/off light layer
    SHADOW_SWITCH = 1402       # Turn on/off shadow layer
    PARALLAX_SWITCH = 1403     # Turn on/off parallax layer
    GROUND_SWITCH = 1404       # Turn on/off ground layer
    ...
    # Default: 1
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    LIGHT_VARIABLE = 848      # Switch to another light
    SHADOW_VARIABLE = 849     # Switch to another shadow
    PARALLAX_VARIABLE = 850   # Switch to another parallax
    GROUND_VARIABLE = 851     # Switch to another ground
...
class Spriteset_Map
  ...
  def update_om_shadow
    ...
    if $game_switches[780] == true 
      if @current_shadow != 99
    ...
    elsif @current_shadow != $game_variables[YSA::OVERLAY::SHADOW_VARIABLE]
      filename = YSA::OVERLAY::SHADOW
      filename += $game_map.map_id.to_s
