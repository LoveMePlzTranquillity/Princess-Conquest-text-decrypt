...
class Game_Player < Game_Character
  ...
  def perform_transfer
    ###gamepad default
    $game_variables[1463] = 0
    $game_variables[1464] = 1
    $game_variables[1465] = 3
    $game_variables[1466] = 2
    $game_variables[1468] = 4
    $game_variables[1469] = 5
    ###gamepad default
    $game_map.delete_particles
    $game_map.delete_skills
    $game_map.delete_damage_sprites
    if transfer?
      set_direction(@new_direction)
      if @new_map_id != $game_map.map_id
        $game_map.setup(@new_map_id)
        $game_map.autoplay
        if @new_map_id == 2 || @new_map_id == 70 || @new_map_id == 319 || @new_map_id == 343 || @new_map_id == 344
          $game_switches[3825] = true
        end
      end
      moveto(@new_x, @new_y)
      clear_transfer_info
      ...
      $game_map.screen.pictures[22].erase      
      $game_map.screen.pictures[98].erase
      $game_map.screen.pictures[99].erase
      $game_map.screen.pictures[125].erase #clear achi icon
    end
   $game_switches[1768] = false
   $game_switches[1770] = false   
   $game_switches[2723] = false  
   $game_switches[2569] = false    
end 
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def debug_through?
    $TEST && Input.press?(:CTRL) || $TEST && WolfPad.press?(:L2,0) || $TEST && WolfPad.press?(:L2,1)
  end
  #--------------------------------------------------------------------------
  ...
  def move_by_input
    ...
    #dir = WolfPad.lstick4 != 0 ? WolfPad.lstick4 : Input.dir4
  ...
  def update_nonmoving(last_moving)
    ...
    if movable? && Input.trigger?(:C) or movable? && WolfPad.trigger?(:X,0) or movable? && WolfPad.trigger?(:X,1)
