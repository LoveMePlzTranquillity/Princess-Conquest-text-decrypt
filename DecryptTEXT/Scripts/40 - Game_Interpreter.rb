...
class Game_Interpreter
  ...
  def command_111
    ...
    when 1  # Variable
      ...
      l1 = value1.length rescue 0
      l2 = value2.length rescue 0      
      case @params[4]
    ...
    when 12  # Script
   #   @splitStr = @params[1].split()
   #   @compSafe = [">","<",">=","<="]
      #####
   #   @i = 1
   #   stopEval = 0
   #   if @splitStr[0].include?"$game_var"
   #   while @i < @splitStr.length
   #     if @compSafe.include?@splitStr[@i]
   #       @iv1 = @i - 1
   #       @iv2 = @i + 1 
   #       @andor = @i + 2
   #       saf1 = @splitStr[@iv1].scan(/\d+/).first
   #       saf2 = @splitStr[@iv2].scan(/\d+/).first
   #       l1 = $game_variables[saf1.to_i].length rescue 0
   #       l2 = $game_variables[saf2.to_i].length rescue 0
   #       if l1 != l2
   #         print("Compared string to Var in SCRIPT")
   #         stopEval = 1
   #         result = false
   #       end  
   #       end
   ...
   #  end
   #  if stopEval == 0
       #puts(@params[1].to_s) #CRASH TEST
       if @params[1].include?"@w"
         if @params[1].include?"-="
         puts(@params[1].to_s) #CRASH TEST
         if @w.nil?
           @w = 0
  ...
  def command_201
    ...
    Fiber.yield while $game_player.transfer? || $game_message.visible || $game_switches[3825] == true || $game_switches[3826] == true || $game_switches[3038] == true || $game_switches[2481] == true  || $game_switches[3907] == true #Achi/War Dec/Daily events/WM Init/TF Suspend
    Cache.clear
    #$game_temp.reserve_common_event(514)
    if $game_switches[739] == true && $game_switches[1151] == true #transmute end regardless of leader
    ...
    if $game_switches[1592] == false && map_id != $game_map.map_id
      weather_stop #weather_ex Stop
    end    
  ...
  def command_355
    ...
    #puts(script.to_s) #fix script
    ### INIT
       if script.include?"@w"
         if script.include?"-="
          #puts(script.to_s) #CRASH TEST
         if @w.nil?
           @w = 0
         end
         end
