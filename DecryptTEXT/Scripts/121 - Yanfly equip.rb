...
module YEA
  module EQUIP
    ...
    DEFAULT_BASE_SLOTS = [ 0, 1, 2, 3, 4, 5]
...
class Game_Actor < Game_Battler
  ...
  #--------------------------------------------------------------------------
    ...
    equip_slots.size.times do |i|
      #not optimizing self-locking equipments
      @selfFix = [671,672,673,692,699]
      @lock = 0
      next if !equip_change_ok?(i)
      next unless can_optimize?(i)
      items = $game_party.equip_items.select do |item|
      if item.is_a?(RPG::Armor)
        if @selfFix.include?item.id
          @lock = 1
        end  
      end  
        item.etype_id == equip_slots[i] &&
        equippable?(item) && item.performance >= 0 && @lock != 1
      end
...
class Window_EquipStatus < Window_Base
  ...
  def draw_current_param(dx, dy, param_id)
    ...
    if param_id == 7 #Luck as letter
      if @actor.id == 1 #Knight
        @lukScore = (@actor.luk / (28 + @actor.level)).round()
        @lukScore *= 1.52
      else #Not Knight
        @lukScore = (@actor.luk / (18 + @actor.level)).round() 
      end         
      if @lukScore < 0.53
      @ll = 0
      elsif @lukScore <= 1.05
      ...
      @ll = 6
      elsif @lukScore <= 4.30 #+0.1
      @ll = 7
      elsif @lukScore <= 4.93 #+0.2
      @ll = 8
      elsif @lukScore <= 5.65 #+0.4
      @ll = 9
      else
      @ll = 10
      end  
      if @actor.equips.include?$data_armors[686] #SackOGold
        @ll += 3
      end 
      if @actor.equips.include?$data_armors[696] #BankerKey
        @ll -= 1
      end       
      if @actor.equips.include?$data_armors[718] #PrinceTrophy
        @ll += 1
      end   
      if @actor.equips.include?$data_armors[719] #ChestLid
        @ll += 2
      end       
      if @actor.equips.include?$data_weapons[151] #MasonHammer
        @ll += 1
      end           
      if @actor.id == 1 #Knight
       if $game_variables[628] == 105 || $game_variables[628] == 1005
        @ll += 2
       end    
      else
        @ll += @actor.parent_stat(3) #wealth
      end      
      if @ll >= 10
        @ll = 10
      end
      ...
      @LUKstring = ["H","G","F","E","D","C","B","A","S","SS","SSS"]
      @LUKlet = @LUKstring[@ll]
      @LUKwri = (@actor.luk).to_s + " (" + @LUKlet + ")"       
      #@LUKwri = (@actor.luk).to_s + " (    " + @LUKlet + " )"    
      #@icx = (@LUKwri.length * 12) - ((@LUKlet.length + 2)*12)
      #draw_icon(1349,@icx,dy)
      dw += 22
      draw_text(20, dy, dw, line_height, @LUKwri, 2)
    else #not Luck
...
class Window_EquipSlot < Window_Selectable
  ...
  def draw_item(index)
    ...
    draw_text(rect.x, rect.y, 128, line_height, slot_name(index))
...
class Scene_Equip < Scene_MenuBase
  ...
  def command_optimize
    ...
    if $game_switches[3343] == false
     $game_switches[1515] = true     
    $game_temp.reserve_common_event(686)
    end
  end
