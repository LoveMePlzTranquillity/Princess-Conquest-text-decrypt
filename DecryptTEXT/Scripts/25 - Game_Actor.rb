...
class Game_Actor < Game_Battler
  ...
  def level_up
    @level += 1
    if $game_switches[3826] == false && $game_switches[3038] == false #no daily going / egg hatch
     $game_temp.reserve_common_event(870)
    end
    self.class.learnings.each do |learning|
      learn_skill(learning.skill_id) if learning.level == @level
    end
