...
class Window_Message < Window_Base
  ...
  def skip_key_pressed?
    !$game_switches[TH::Message_Skip::Disable_Switch] && Input.press?(skip_key) || !$game_switches[TH::Message_Skip::Disable_Switch] && WolfPad.press?(:R2,0) || !$game_switches[TH::Message_Skip::Disable_Switch] && WolfPad.press?(:R2,1)
  end
  ...
  def input_pause
    self.pause = true
    wait(10)
    Fiber.yield until Input.trigger?(:B) || Input.trigger?(:C) || skip_key_pressed? || WolfPad.trigger?(:X,0) || WolfPad.trigger?(:B,0) || WolfPad.trigger?(:X,1) || WolfPad.trigger?(:B,1)
    Input.update
    self.pause = false
  end
  ...
  def wait(duration)
    return if TH::Message_Skip::Skip_Delays && skip_key_pressed? && $game_switches[3000] == false
