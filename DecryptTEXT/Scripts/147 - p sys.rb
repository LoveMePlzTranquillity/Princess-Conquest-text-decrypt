...
module YEA
  module PARTY
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ENABLE_MENU = false   # Enables party menu. Default: false
...
class Window_PartySelect < Window_Selectable
  ...
  def process_handling
    ...
    return process_cancel   if cancel_enabled?    && Input.trigger?(:B) or cancel_enabled? && WolfPad.trigger?(:B,0) or cancel_enabled? && WolfPad.trigger?(:B,1)
